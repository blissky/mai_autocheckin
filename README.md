...
- **识图验证**：可选接入多模态大模型，通过截图识别判断签到是否成功
...
| 配置 | 类型 | 默认值 | 说明 |
|------|------|--------|------|
| `enabled` | bool | true | 是否启用插件功能（WebUI 与定时签到） |
| `config_version` | string | `1.0.0` | 配置版本，勿手动修改 |
...
| `vision_model` | string | "" | 识图使用的模型名称，留空使用宿主默认模型（需支持图像输入） |
...
识图通过 MaiBot 的 LLM 接口调用多模态模型，图像以 OpenAI 风格的 `image_url`（base64 data URL）消息体传入。`vision_model` 留空时使用宿主默认模型——请确保所选模型支持图像输入，可先在 WebUI「识图选区」页中用「识图测试」验证效果。
...
- **识图调用失败**：确认宿主已配置支持图像输入的多模态模型；多模态消息体格式依宿主实现可能存在差异，可用 WebUI「识图测试」定位问题。
