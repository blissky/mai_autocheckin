...
1. 插件加载后，通过宿主映射的 WebUI 端口打开控制面板（默认监听 `0.0.0.0:9010`，便于从容器外部访问）
...
| `host` | string | `0.0.0.0` | WebUI 监听地址，默认允许从容器外部访问 |
...
- **网络暴露**：WebUI 默认监听 `0.0.0.0`，请勿将端口直接暴露到公网；建议限制端口访问并通过 HTTPS 反向代理转发
